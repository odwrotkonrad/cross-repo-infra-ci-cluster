...
resource "google_cloud_quotas_quota_preference" "cpus_all_regions" {
  parent   = "projects/${var.ci_project_id}"
  ...
  #[why] the applier's cloudquotas permissions come from the custom role bound in
  #   cross-repo/infra/base, so there is no local resource to order against: only the service
  #   enablement is ours to wait for
